...
\subsection{IPv4 routing}\label{ipv4-routing}}

\hypertarget{nat}{%
\subsubsection{NAT}\label{nat}}

\hypertarget{nat-traversal}{%
\subsection{NAT Traversal}\label{nat-traversal}}
...
and encryption. The \texttt{Address} field specifies the virtual IP
address that the local network interface will use, while the
...
be configured with a reachable endpoint for the other peer. In the above
