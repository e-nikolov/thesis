...
(\glsxtrshort[hyper=true]{ftp}), email (\glsxtrshort[hyper=true]{smtp}),
instant messaging, remote access (\glsxtrshort[hyper=true]{ssh}
\autocite{sshRFC}) and others. The Web is a collection of interconnected
documents that use Web technologies such as
\glsxtrshort[hyper=true]{html} and JavaScript. It is typically accessed
...
The \glsxtrfull{ip} \autocite{ipv4RFC} is a Network layer (L3) protocol
...
\glsxtrfull{udp} is a very thin Transport layer (L4) protocol that only
...
of secure connections, QUIC is tightly coupled with \glsxtrshort{tls}
version 1.3. One of the main uses of QUIC is inside \glsxtrshort{http}
version 3.
\end{block}
...
\glsxtrshort{quic}. HTTP provides several features such as:
...
\glsxtrfull{tls} \autocite{tlsRFC} and its precursor \gls{ssl} are
...
attacks. A certificate includes information such as: - Subject - an
identifiable name for the certificate's owner. Depending on the use case
it can be a domain name, an IP address, an email address or others. -
...
decrypt. - Issuer - an entity that is responsible for validating the
identity of the subject. It is usually a \gls{ca} that is trusted by the
consumer of the certificate, but in the case of a self-signed
certificate, it can be the subject itself - Issuer's signature - a
signature of the certificate's contents using the issuer's private key
...
to secure the underlying \glsxtrshort{tcp} or \glsxtrshort{quic}
...
architectures and TLS-based \glsxtrshort{vpn} applications.
...
\glsxtrshort{isp} or an intruder with access to the network cannot
decrypt the traffic to see what is being communicated, it could see
which IP addresses are communicating with each other.
\end{block}

\begin{block}{Network Layer (L3) and Data Link Layer (L2) Security
Protocols}
\protect\hypertarget{network-layer-l3-and-data-link-layer-l2-security-protocols}{}
\begin{block}{IPSec}
\protect\hypertarget{ipsec}{}
\glsxtrfull{ipsec} is a protocol suite that provides secure
communications at the Network layer (L3). Unlike \glsxtrshort{tls},
...
\item
  ...
  hosts. Similar to \glsxtrshort{tls}, routers between the hosts can
...
\item
  ...
  used to securely connect a \glsxtrshort{vpn} client to a server that
...
OpenVPN and a key management approach inspired by \glsxtrshort{ssh}. It
...
Protocols such as \glsxtrfull{turn} \autocite{turnRFC} and
\glsxtrfull{derp} \autocite{derpDocs} can be used to securely implement
relaying.
...
\glsxtrshort{tun} or \glsxtrshort{tap}.
