...
\phantomsection\label{thesis__090-mpyc-web.md__ecosystem-availability}
From the listed
\end{block}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ assign(iters}\OperatorTok{=}\DecValTok{100\_000}\NormalTok{):}
    \ControlFlowTok{for}\NormalTok{ \_ }\KeywordTok{in}\NormalTok{ itertools.repeat(}\VariableTok{None}\NormalTok{, iters):}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ multiply(iters}\OperatorTok{=}\DecValTok{100\_000}\NormalTok{):}
    \ControlFlowTok{for}\NormalTok{ \_ }\KeywordTok{in}\NormalTok{ itertools.repeat(}\VariableTok{None}\NormalTok{, iters):}
        \DecValTok{17} \OperatorTok{*} \DecValTok{41}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ fibonacci(n}\OperatorTok{=}\DecValTok{100\_000}\NormalTok{):}
...
    \ControlFlowTok{for}\NormalTok{ \_ }\KeywordTok{in}\NormalTok{ itertools.repeat(}\VariableTok{None}\NormalTok{, iters):}
\NormalTok{        a, b }\OperatorTok{=}\NormalTok{ b, (a }\OperatorTok{+}\NormalTok{ b) }\OperatorTok{\%} \DecValTok{100\_000}
...
\begin{block}{Startup}
\phantomsection\label{thesis__090-mpyc-web.md__startup}
\end{block}

\begin{block}{Conclusions}
\phantomsection\label{thesis__090-mpyc-web.md__conclusions}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.1122}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.1531}}
...
\begin{block}{MPyC Web}
\phantomsection\label{thesis__090-mpyc-web.md__mpyc-web-1}
This chapter presents the design of MPyC Web - an MPC connectivity solution based on web technologies (L7) that can run in web browsers.
...
\begin{block}{Python Runtime Selection}
\phantomsection\label{thesis__090-mpyc-web.md__python-runtime-selection-1}
\begin{block}{Selection Criteria}
\phantomsection\label{thesis__090-mpyc-web.md__selection-criteria-1}
While web browsers natively support only \gls{js}, there are approaches with different tradeoffs that can enable other languages, including Python. In this section we will investigate several web-based Python runtimes and choose the most appropriate one for MPyC Web.
...
\begin{block}{Available Solutions}
\phantomsection\label{thesis__090-mpyc-web.md__available-solutions-1}
There are a number of available solutions for using Python in web browsers, falling under three main categories\autocite{pyodideIntroMozilla} \autocite{anvilPythonBrowser}:
...
\begin{block}{Ecosystem Availability}
\phantomsection\label{thesis__090-mpyc-web.md__ecosystem-availability-1}
From the listed
\end{block}

\begin{block}{Preliminary Runtime Benchmarks}
\phantomsection\label{thesis__090-mpyc-web.md__preliminary-runtime-benchmarks-1}
We will measure the time it takes each Python runtime to execute a given set of benchmark functions to have a preliminary indication of how they might perform for MPyC. To put that data in context, we will also include the results for the native performance of Python's default implementation (CPython) as well as JavaScript running in a browser.
...
\begin{block}{Startup}
\phantomsection\label{thesis__090-mpyc-web.md__startup-1}
\end{block}

\begin{block}{Conclusions}
\phantomsection\label{thesis__090-mpyc-web.md__conclusions-1}
Out of the discussed runtime options, Pyodide offers the widest support for packages from the standard library and third-party packages. None of the options support the sockets package due to browser limitations, but an alternative implementation can be built on top of the WebRTC API.
...
\begin{block}{Design of MPyC Web}
\phantomsection\label{thesis__090-mpyc-web.md__design-of-mpyc-web-1}
Web based solution for running MPC in browsers on the client side via WebAssembly and WebRTC for peer-to-peer connectivity.
...
\begin{block}{Implementation details}
\phantomsection\label{thesis__090-mpyc-web.md__implementation-details-1}
\end{block}

\begin{block}{Performance analysis}
\phantomsection\label{thesis__090-mpyc-web.md__performance-analysis-1}
\end{block}

\begin{block}{Security analysis}
\phantomsection\label{thesis__090-mpyc-web.md__security-analysis-1}
\begin{block}{Trust model}
\phantomsection\label{thesis__090-mpyc-web.md__trust-model-1}
There still is a centralized service like in the Tailscale solution, but here it is self-deployed.
\end{block}

\begin{block}{Identity}
\phantomsection\label{thesis__090-mpyc-web.md__identity-1}
\end{block}
\end{block}

\begin{block}{Usability analysis}
\phantomsection\label{thesis__090-mpyc-web.md__usability-analysis-1}
\end{block}
\end{block}
