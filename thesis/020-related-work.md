...
Another approach for NAT traversal that usually achieves good performance, but only works with some types of NATs is to use **\gls{stun}** in combination with UDP hole punching. 

STUN is a set of protocols that allows for a device to detect if there is a NAT gateway that connects it to The Internet and if so to determine the type of the NAT and its public IP address. This is achieved by sending UDP datagrams to a public STUN server which responds with the source IP address and port it sees in them. 

Performs several checks to determine the behavior of the NAT gateway, e.g. ip mapping, port mapping, unique ip 

If a NAT rewrote the datagrams and then informs the client of them.  UDP hole punching is a related technique that 

STUN is a set of protocols that allows for a public STUN server to be contacted by a client device to determine the public IP address and the behavior of the NAT gateway that connects it to The Internet. UDP hole punching is a related technique 
