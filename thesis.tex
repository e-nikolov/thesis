...
\begin{figure}^^A
    \begin{center}
        \includegraphics[width=0.5\textwidth]{#3}^^A
        \caption{#4}^^A
        \label{#1}^^A
    \end{center}
\end{figure}^^A
...
\markdownSetup{theme=witiko/graphicx/http}


\def\markdownRendererJekyllDataStart{ }
...
% \def\markdownOptionHelperScriptFileName{thesis2.markdown.lua}
\def\markdownOptionOutputDir{build}
\def\markdownOptionCacheDir{build}
...
\markdownInput{thesis/01-testing.md}
% \markdownInput{prep/02-goals.md}

\setkeys{Gin}{scale=0.25}
% \markdownInput{prep/03-technical-survey.md}
% % \markdownInput{prep/04-design.md}
% \markdownInput{prep/05-implementation.md}
% \markdownInput{prep/06-conclusions.md}
